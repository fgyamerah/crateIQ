"""
Tests for single-track and bulk Artist/Title/Genre/Album editing on managed Inbox
...
def test_patch_route_edits_all_supported_metadata_fields_together(client):
    test_client, root = client
    track_id = _seed_track(root, "song.mp3")

    resp = test_client.patch(
        f"/api/workspace/inbox/tracks/{track_id}",
        json={"artist": "Black Coffee", "title": "New Title", "genre": "Afro House", "album": "New Album"},
    )
    ...
    assert _read_tags(root / "Inbox" / "song.mp3") == {"artist": "Old Artist", "title": "Some Title", "album": "", "genre": "Dance"}
...
def test_bulk_edit_apply_sets_genre_db_only_and_marks_unsaved(managed_root):
    ...
    for track_id, filename in zip(ids, ["t0.mp3", "t1.mp3", "t2.mp3"]):
        assert _row(managed_root, track_id)["genre"] == "Afro House"
        assert _read_tags(managed_root / "Inbox" / filename)["genre"] == "Dance"
        item = next(item for item in result["results"] if item["track_id"] == track_id)
        assert item["preparation_state"]["status"] == "UNSAVED"
...
def test_bulk_edit_apply_sets_artist_across_several_tracks(managed_root):
    ...
    for track_id in ids:
        assert _row(managed_root, track_id)["artist"] == "Black Coffee"
    assert all(item["preparation_state"]["pending_fields"] == ["artist"] for item in result["results"])


def test_bulk_edit_apply_sets_mixed_fields_in_one_db_first_operation(managed_root):
    ids = [_seed_track(managed_root, f"t{i}.mp3") for i in range(2)]

    result = workspace_service.bulk_edit_apply(
        managed_root, ids, artist="Black Coffee", title="New Title", genre="Afro House", album="New Album", confirm=True,
    )

    assert result["succeeded_count"] == 2
    for track_id, filename in zip(ids, ["t0.mp3", "t1.mp3"]):
        row = _row(managed_root, track_id)
        assert row["artist"] == "Black Coffee" and row["title"] == "New Title"
        assert row["genre"] == "Afro House" and row["album"] == "New Album"
        tags = _read_tags(managed_root / "Inbox" / filename)
        assert tags == {"artist": "Old Artist", "title": "Some Title", "album": "", "genre": "Dance"}
...
def test_bulk_edit_apply_one_failure_does_not_flip_other_successes(managed_root):
    """An unsupported-format track remains editable but stays write-blocked."""
    ...
    assert by_id[bad_id]["status"] == "succeeded"
...
def test_bulk_edit_apply_refreshes_readiness(managed_root):
    ...
    assert all(item["preparation_state"]["status"] == "UNSAVED" for item in after["items"])
...
def test_bulk_edit_apply_route_applies_with_confirm(client):
    ...
    assert body["tag_write"] is None


def test_bulk_edit_route_rejects_more_than_maximum_selection(client):
    test_client, _ = client
    resp = test_client.post(
        "/api/workspace/inbox/bulk-edit/apply",
        json={"track_ids": list(range(1, 202)), "artist": "Black Coffee", "confirm": True},
    )
    assert resp.status_code == 422
