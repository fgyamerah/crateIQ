...
import config
import db
from utils import path_reconciliation as _path_reconciliation
from modules import (
...
def assert_path_under_root(path: Path | str, root: Path | str) -> Path:
    ...
    Delegates to utils.path_reconciliation so this check has a single
    implementation shared with the backend.
    """
    return _path_reconciliation.assert_path_under_root(path, root)
...
# ---------------------------------------------------------------------------
_path_audit_db_path = _path_reconciliation._path_audit_db_path
_path_audit_table_columns = _path_reconciliation._path_audit_table_columns
_PATH_AUDIT_STAGE_PRIORITY = _path_reconciliation._PATH_AUDIT_STAGE_PRIORITY
_path_audit_orphan_candidates = _path_reconciliation._path_audit_orphan_candidates
_path_audit_orphan_candidate_tier = _path_reconciliation._path_audit_orphan_candidate_tier
...
_path_audit_best_rename_match = _path_reconciliation._path_audit_best_rename_match


def _path_audit_report(root: Path, db_path: Path, *, include_orphan_candidates: bool = False) -> dict:
    """Compatibility wrapper around the neutral path-audit engine.

    Delegates to utils.path_reconciliation.path_audit_report, injecting this
    CLI's config.py-derived audio-extension/skip-dir settings so
    config_local.py overrides keep working for pipeline.py callers. The
    backend calls utils.path_reconciliation.path_audit_report directly and
    does not go through this wrapper.
    """
    return _path_reconciliation.path_audit_report(
        root,
        db_path,
        include_orphan_candidates=include_orphan_candidates,
        audio_extensions=config.AUDIO_EXTENSIONS,
        skip_dirs=config.MAINTENANCE_SKIP_DIRS,
    )
...
# ---------------------------------------------------------------------------
_path_reconcile_plan = _path_reconciliation.path_reconcile_plan
...
_path_reconcile_validate_plan = _path_reconciliation.path_reconcile_validate_plan
...
_path_reconcile_latest_plan_path = _path_reconciliation.path_reconcile_latest_plan_path
...
def run_validate_docs(args) -> int:
    ...
    Deliberately does not call _setup_logging(): this command only reads
    COMMANDS.txt and the in-process doc registry (no log.*() calls occur in
    this code path), and _setup_logging() would otherwise create
    config.LOGS_DIR under MUSIC_ROOT -- coupling a pure docs check to music
    root availability for no benefit.
    """
...
def main() -> None:
    ...
    print(
        "NOTE: The bare `pipeline.py` automatic workflow is legacy "
        "compatibility tooling. Current crateIQ preparation uses "
        "Managed Workspace -> Inbox -> Process All in the FastAPI app.",
        file=sys.stderr,
    )
