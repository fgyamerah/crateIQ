      ...
      {loading ? <p className="muted">Loading optional workflows…</p> : <div className="analysis-jobs-grid">
        {filteredJobs.map((job) => <article className="analysis-job-card" id={job.type.replace(/_/g, '-')} key={job.type}>
          ...
          <div className="settings-action-row">
            ...
            {job.type === 'mixed_in_key_coverage' ? <Link className="btn btn--primary btn--sm" to="/settings#analysis-tools">Open MIK import</Link> : (job.type === 'bpm_analysis' || job.type === 'key_analysis') && job.runner_implemented ? <span className="analysis-job-run-hint">Preview, then confirm below</span> : job.type === 'duplicate_detection' ? <Link className="btn btn--ghost btn--sm" to="/duplicates"><Eye size={13} /> Duplicate Review</Link> : job.type === 'audio_quality_probe' ? <Link className="btn btn--ghost btn--sm" to="/quality-review"><Eye size={13} /> Quality Review</Link> : <Link className="btn btn--ghost btn--sm" to="/settings#analysis-tools"><Wrench size={13} /> {job.status === 'missing_tool' ? 'Tool setup' : job.type === 'beets_enrichment' ? 'Review required' : 'Runner pending'}</Link>}
          </div>
        </article>)}
      </div>}
