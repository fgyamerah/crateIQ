...
import type { CrateDetail, CrateSummary, CrateTransition } from '../types/crate'
...
const transitionTone: Record<CrateTransition['label'], BadgeTone> = { smooth: 'succeeded', workable: 'pending', clash: 'failed', unknown: 'cancelled' }
const transitionLabel: Record<CrateTransition['label'], string> = { smooth: 'Smooth', workable: 'Workable', clash: 'Clash', unknown: 'Unknown' }
    ...
    <div className="manual-crates-layout">
      ...
      <section className="manual-crate-detail">
        ...
        {detail && <>
          ...
          {detail.tracks.length === 0 ? <EmptyState icon={<ListMusic size={24} />} title="This crate is empty" message="Use the library finder below to add tracks. Their order stays explicit and editable." /> : <div className="manual-crate-tracks">{detail.tracks.map((track, index) => <Fragment key={track.track_id}><div className="manual-crate-track"><span className="manual-crate-position">{track.position}</span><div className="manual-crate-track-main"><strong>{trackTitle(track)}</strong><span>{track.genre ?? 'Unknown genre'} · {track.bpm?.toFixed(1) ?? '—'} BPM · {fmtDuration(track.duration_sec)}{track.missing_from_library ? ' · no longer in selected library' : ''}</span></div>{track.key_camelot && <Badge tone="info">{track.key_camelot}</Badge>}<ReviewStatusBadge trackId={track.track_id} status={reviewSummary[String(track.track_id)]?.review_status} rating={reviewSummary[String(track.track_id)]?.rating} /><div className="manual-crate-track-actions"><button className="btn btn--ghost btn--xs" disabled={saving || track.missing_from_library} onClick={() => setPreviewTrack({ id: track.track_id, artist: track.artist, title: track.title, filename: track.filename, genre: track.genre, bpm: track.bpm, key_camelot: track.key_camelot, duration_sec: track.duration_sec })}>Preview</button><button className="btn btn--ghost btn--xs" disabled={saving || index === 0} aria-label={`Move ${trackTitle(track)} up`} onClick={() => void moveTrack(index, -1)}><ChevronUp size={15} /></button><button className="btn btn--ghost btn--xs" disabled={saving || index === detail.tracks.length - 1} aria-label={`Move ${trackTitle(track)} down`} onClick={() => void moveTrack(index, 1)}><ChevronDown size={15} /></button><button className="btn btn--ghost btn--xs" disabled={saving} onClick={() => void removeTrack(track.track_id)}>Remove</button></div></div>{track.transition_to_next && <div className={`manual-crate-transition manual-crate-transition--${track.transition_to_next.label}`}><Badge tone={transitionTone[track.transition_to_next.label]}>{transitionLabel[track.transition_to_next.label]}</Badge><span>{track.transition_to_next.explanation}</span></div>}</Fragment>)}</div>}
