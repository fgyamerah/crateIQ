...
export default function SmartCrates() {
  ...
  return <main className="crate-workspace manual-crates-page smart-crates-page">
    ...
    <div className="smart-crates-layout">
      ...
      {preview ? <section className="smart-preview"><div className="manual-crate-section-head"><div><h2>{preview.generated_name}</h2><p>{preview.explanation}</p></div><Badge tone="info">{preview.track_count} tracks</Badge></div>{preview.warnings.map((warning) => <StatusStrip key={warning} tone="warn">{warning}</StatusStrip>)}{preview.tracks.length ? <div className="manual-crate-tracks">{preview.tracks.map((track) => <div className="manual-crate-track" key={track.track_id}><span className="manual-crate-position">{track.position}</span><div className="manual-crate-track-main"><strong>{titleOf(track)}</strong><span>{track.reasons.join(' · ')}</span></div>{track.bpm != null && <Badge tone="pending">{track.bpm.toFixed(1)} BPM</Badge>}{track.key_camelot && <Badge tone="info">{track.key_camelot}</Badge>}{track.genre && <Badge tone="running">{track.genre}</Badge>}</div>)}</div> : <EmptyState icon={<WandSparkles size={24} />} title="No matching tracks" message="Broaden the criteria, then generate another local preview." />}</section> : <EmptyState icon={<WandSparkles size={26} />} title="Build a preview" message="Choose a preset or criteria, then preview the suggested ordered track list before saving." />}</section>
    </div>
  </main>
}
