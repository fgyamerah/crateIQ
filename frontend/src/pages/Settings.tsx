...
export default function Settings() {
  ...
  return (
    <div className="page settings-page">
      ...
      />
      <StatusStrip tone="info" icon={<Settings2 size={15} />} footnote="Local-first diagnostics · CrateIQ never touches your DJ application's live database.">
        Imported originals remain untouched. Managed Inbox copies can be cleaned and tagged after explicit processing, and promotion moves the managed copy into Library.
      </StatusStrip>
      ...
      {loading ? <EmptyState title="Loading settings" message="Reading local runtime diagnostics…" /> : !settings ? <EmptyState title="Settings unavailable" message="Check that the local backend is running, then recheck readiness." /> : (
        <>
          ...
          <WorkspacePanel settings={settings} workspace={workspace} onReload={load} />
          ...
          <section className="section settings-anchor-section" id="advanced">
            <div className="card settings-card settings-advanced-card">
              <div className="card-title-row">
                <div><h2 className="card-title"><Wrench size={16} /> Advanced</h2><p className="muted">Runtime path diagnostics and legacy direct-library controls. Not needed for the normal Managed Workspace flow.</p></div>
                <Badge tone="info">Rarely needed</Badge>
              </div>
              <dl className="def-list settings-def-list">
              ...
              <details className="settings-legacy-details" id="legacy-direct-library">
                <summary>Legacy Direct Library</summary>
                <StatusStrip tone="warn" icon={<CircleAlert size={15} />}>
                  Legacy mode scans and indexes music in place on the active library root and does not provide the managed Inbox → Library workflow. For new setups, Managed Workspace (above) is recommended.
                </StatusStrip>
                <div className="settings-import-wizard">
                  <div className="card-title-row">
                    <div><h3 className="card-title">Legacy setup &amp; import</h3><p className="muted">Follow the steps in order. Optional BPM/key analysis always happens later, never during import.</p></div>
