...
- Settings (`/settings`): central local diagnostics for the active library, optional tools, readiness checks, and locked safety policies. The only editable setting today is the default export path mode; it is saved locally as `<library-root>/logs/app_settings.json` and initializes export forms on their next load. Library-root and binary overrides stay private process-start configuration, so Settings provides restart guidance instead of live editing. No files, tags, MIK values, or DJ databases are changed.
